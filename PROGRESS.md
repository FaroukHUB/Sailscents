...
- **Déploiement Vercel** : en cours (`sailscents.vercel.app`). Voir point bloquant ci-dessous concernant le schéma de base de données.
- **Vraies migrations Payload** : à générer dès que le CLI Payload fonctionnera dans un environnement de build (voir point bloquant ci-dessous), pour remplacer la route `/api/internal/bootstrap-db` par le mécanisme standard et supprimer cette dernière.

---

## 5. Points bloquants / à noter

- **`npm run generate:types` et `payload migrate:create` échouent actuellement dans cet environnement** avec une erreur Node.js (`ERR_REQUIRE_ASYNC_MODULE`) liée à l'interaction entre le chargeur `tsx` de Payload et Node 22 sur `@payloadcms/richtext-lexical`. Cela **n'affecte ni `npm run dev`, ni `npm run build`, ni l'admin** (tous validés fonctionnels) — uniquement les commandes CLI Payload (types et migrations). Conséquence directe : impossible de générer de vraies migrations Payload pour la production dans cet environnement ; voir le point suivant pour le contournement mis en place.
- **Payload ne synchronise le schéma Postgres automatiquement qu'en développement** (`NODE_ENV !== 'production'`, en dur dans `@payloadcms/db-postgres`, non contournable via une option de configuration). Une base de production fraîchement provisionnée (Neon) reste donc **vide** tant qu'aucune migration n'a été appliquée. C'est ce qui a provoqué l'erreur `relation "users" does not exist` sur le premier déploiement Vercel.
  **Contournement mis en place** : `GET /api/internal/bootstrap-db?secret=PAYLOAD_SECRET` appelle directement `pushDevSchema` (le même mécanisme qu'en dev), en contournant le garde-fou `NODE_ENV`. À utiliser **une seule fois** après le premier déploiement (ou après tout changement de schéma tant que les migrations ne sont pas en place), puis **à supprimer** dès que de vraies migrations Payload auront pu être générées.
